...
class vivo():
    ...
    def shuchu(self,date):
        ...
        #writer = pd.ExcelWriter('vivo商店.xlsx')
        writer = pd.ExcelWriter(r'安卓商店\vivo商店.xlsx')
        # -------------------------------影音视听-----------------------------------------
        # 未登录只能看前4页200条数据
        data_dict2 = {
            'app名称': [], '排名': [], '排名变化': [], '新增下载量': [], '类别': [], '评分': [], '公司': [], '更新日期': [],
        ...
        for page in range(1, 2):
            ...
            # 将df1与df2写入writer中
            df2.to_excel(writer, sheet_name='应用榜', index=False, encoding='utf-8')
        ...
        for page in range(1, 2):
            ...
            #writer = pd.ExcelWriter('xiaomi.xlsx')
            # 将df1与df2写入writer中
            df3.to_excel(writer, sheet_name='热搜榜', index=False, encoding='utf-8')
        ...
        #     #writer = pd.ExcelWriter('xiaomi.xlsx')
        #     # 将df1与df2写入writer中
        #     df4.to_excel(writer, sheet_name='社交通讯', index=False, encoding='utf-8')
        #
        # # -------------------------------图书阅读-----------------------------------------
        # data_dict5 = {
        #     'app名称': [], '排名': [], '排名变化': [], '新增下载量': [], '类别': [], '评分': [], '公司': [], '更新日期': [],
